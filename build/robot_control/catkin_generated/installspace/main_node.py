...
class MainController:
    ...
    主控制器节点：
      - 订阅：
          ~/start_cmd_topic   (默认 /ee_pose)          —— 当前末端位姿（作为起点来源）
    ...
    注：不额外新增成员变量，复用 pending_target 作为“缓存槽位”。
    """

    def __init__(self):
        # —— 可配置话题名 —— #
        self.start_cmd_topic    = rospy.get_param("~start_cmd_topic", "/ee_pose")
    ...
    def target_cb(self, msg: PoseStamped):
        ...
        with self._lock:
            self.pending_target = msg  # 缓存目标（用作“开门”信号与数据承载）
